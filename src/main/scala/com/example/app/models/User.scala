...
case class UserLogin(username: Option[String], email: Option[String], password: String){
  require({(username ++ email).size > 0}, "Must provide either a username or email")
...
object User extends Updatable[User, (Int, String, String, String), Tables.Users]{
  ...
  def uniqueUsername(username: String) =
    db.run(table.filter(_.username === username).result).map(_.size == 0)

  def uniqueEmail(email: String) =
    db.run(table.filter(_.email === email).result).map(_.size == 0)
  ...
  def createNewUser(userCreate: UserCreate) = {
    ...
    Future.sequence(Seq(usernameIsUnique, emailIsUnique)).flatMap(k => (k(0), k(1)) match {
      case (true, true) =>
